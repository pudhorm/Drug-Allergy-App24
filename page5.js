...
(function () {
  ...
  if (!root.page5) {
    ...
    root.page5.notes = root.page5.notes || { pharmacist: "", doctor: "" };
  }
})();
...
window.renderPage5 = function () {
  ...
  const { drugLines, adrLines, notes } = window.drugAllergyData.page5;
  ...
  pageEl.innerHTML = `
    <div class="p5-wrapper">
      ...
      <div class="p5-timeline-box">
        <h3 style="margin:0 0 4px;">Visual Timeline</h3>
        <div id="p5TimelineScroll" style="margin-top:0;">
      ...
      <!-- ช่องบันทึกโน้ต (อยู่ใต้ Timeline) -->
      <div class="p5-notes" style="margin-top:14px;">
        <div class="p5-note-card">
          <div class="p5-note-title">Pharmacist Note</div>
          <textarea id="p5PharmNote" rows="6" placeholder="บันทึกโดยเภสัชกร...">${notes.pharmacist || ""}</textarea>
        </div>
        <div class="p5-note-card" style="margin-top:14px;">
          <div class="p5-note-title">Doctor Note</div>
          <textarea id="p5DocNote" rows="6" placeholder="บันทึกโดยแพทย์...">${notes.doctor || ""}</textarea>
        </div>
      </div>
  ...
  if (btnAddDrug) {
    btnAddDrug.addEventListener("click", function () {
      window.drugAllergyData.page5.drugLines.push({
        name: "", startDate: "", startTime: "", stopDate: "", stopTime: ""
      });
  ...
  pageEl.querySelectorAll(".p5-line-del").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      if (kind === "adr")  window.drugAllergyData.page5.adrLines.splice(idx, 1);
      if (window.saveDrugAllergyData) window.saveDrugAllergyData();
      p5EmitUpdate("page5");
      window.renderPage5();
    });
  ...
  function safeDrawTimeline() { try { drawTimeline(); } catch (e) { console.error("[page5] drawTimeline error:", e); } }
  ...
  pageEl.querySelectorAll(".p5-drug-card").forEach((card) => {
    ...
    if (nameInput) nameInput.addEventListener("input", (e) => {
      ...
      safeDrawTimeline(); if (window.saveDrugAllergyData) window.saveDrugAllergyData(); p5EmitUpdate("page5");
    });
    ...
    if (startTimeInput) startTimeInput.addEventListener("change", (e) => {
      ...
      if (window.saveDrugAllergyData) window.saveDrugAllergyData(); p5EmitUpdate("page5");
    });
    if (stopInput) stopInput.addEventListener("change", (e) => {
      window.drugAllergyData.page5.drugLines[idx].stopDate = e.target.value;
      safeDrawTimeline(); if (window.saveDrugAllergyData) window.saveDrugAllergyData(); p5EmitUpdate("page5");
    });
    if (stopTimeInput) stopTimeInput.addEventListener("change", (e) => {
  ...
  // ผูกโน้ต 2 ช่อง
  const pharm = document.getElementById("p5PharmNote");
  const doc   = document.getElementById("p5DocNote");
  if (pharm) pharm.addEventListener("input", (e) => {
    window.drugAllergyData.page5.notes.pharmacist = e.target.value;
    if (window.saveDrugAllergyData) window.saveDrugAllergyData();
    p5EmitUpdate("page5");
  });
  if (doc) doc.addEventListener("input", (e) => {
    window.drugAllergyData.page5.notes.doctor = e.target.value;
    if (window.saveDrugAllergyData) window.saveDrugAllergyData();
    p5EmitUpdate("page5");
  });
  ...
  if (clearBtn) {
    clearBtn.addEventListener("click", () => {
      window.drugAllergyData.page5 = { drugLines: [], adrLines: [], notes: { pharmacist:"", doctor:"" } };
      if (window.saveDrugAllergyData) window.saveDrugAllergyData();
      p5EmitUpdate("page5");
      window.renderPage5();
  ...
  if (printBtn) printBtn.addEventListener("click", p5PrintTimeline);
...
function drawTimeline() {
  ...
  drugs.forEach((d, idx) => {
    ...
    const isSameDayExplicit =
      d.startDate && d.stopDate && parseDate(d.startDate) && parseDate(d.stopDate) &&
  ...
  adrs.forEach((a, idx) => {
    ...
    const isSameDayExplicit =
      a.startDate && a.endDate && parseDate(a.startDate) && parseDate(a.endDate) &&
...
// ====== พิมพ์หน้า 5 แบบสี + สรุปข้อความ ======
function p5PrintTimeline() {
  const page = document.getElementById("page5");
  if (!page) { window.print(); return; }

  // ดึงข้อมูลที่กรอกจริง
  const p5 = (window.drugAllergyData && window.drugAllergyData.page5) || { drugLines: [], adrLines: [], notes: { pharmacist:"", doctor:"" } };
        ...
        ${
          drugs.length
            ? `<ol>${drugs.map(d=>{
                  ...
                  const sD=fmtDateTH(d.startDate); const sT=fmtTime(d.startTime);
                  const eD=fmtDateTH(d.stopDate);  const eT=fmtTime(d.stopTime);
        ...
        ${
          adrs.length
            ? `<ol>${adrs.map(a=>{
                  ...
                  const sD=fmtDateTH(a.startDate); const sT=fmtTime(a.startTime);
                  const eD=fmtDateTH(a.endDate);  const eT=fmtTime(a.endTime);
  ...
  win.document.write(`
    <html>
      <head>
        ...
        <style>
          * { box-sizing:border-box; font-family:system-ui,-apple-system,"Segoe UI",sans-serif; }
          ...
          .p5-footer-btns, .p5-btn-group, .p5-form-block { display:none !important; }
          /* ซ่อนโน้ตของหน้าจอ แล้วทำเวอร์ชันสำหรับพิมพ์แทน */
          .p5-notes { display:none !important; }
          ...
          .p5-timeline-box h3 { margin:0 0 4px !important; }
          #p5TimelineScroll { overflow:visible !important; width:auto !important; max-width:none !important; display:inline-block; background:#fff; margin-top:0 !important; }
          ...
          /* กล่องสำหรับเขียนมือหลังพิมพ์ — อยู่ใต้ Visual Timeline */
          .p5-print-notes { margin-top:10px; }
          .p5-print-note-title { font-weight:700; color:#111827; margin:12px 0 4px; }
          .p5-print-note-box { height:120px; border:2px dashed #cbd5e1; border-radius:10px; background:#fff; }
      ...
      <body>
        ...
        <script>
          (function () {
            // ปรับความกว้างกริดของ timeline สำหรับพิมพ์
            ...
            if (box && dateRow && drugLane && adrLane) {
              ...
              // ===== เพิ่มกล่อง Note สำหรับเขียนมือ "ใต้" Timeline =====
              const notes = document.createElement("div");
              notes.className = "p5-print-notes";
              notes.innerHTML = \`
                <div class="p5-print-note-title">Pharmacist Note</div>
                <div class="p5-print-note-box"></div>
                <div class="p5-print-note-title">Doctor Note</div>
                <div class="p5-print-note-box"></div>
              \`;
              box.parentNode.insertBefore(notes, box.nextSibling);
            }
