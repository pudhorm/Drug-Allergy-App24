...
(function () {
  ...
  function collectSignals() {
    ...
    // ตำแหน่ง/การกระจาย/เยื่อบุ
    (p1.locations || []).forEach((loc) => set.add("pos:" + loc));
    if (p1.distribution === "สมมาตร") set.add("pos:สมาตร");
    ...
    // ---- หน้า 3 (อ่านเฉพาะที่ติ๊ก)
    function labChecked(group, item) { var g=p3[group]; var r=g&&g[item]; return !!(r&&r.checked); }
    function labNumber(group, item) {
      var g=p3[group]; var r=g&&g[item]; var v=r&&r.value;
      var n=Number((v||"").toString().replace(/[, ]+/g,"")); return Number.isFinite(n)?n:NaN;
    }
    if (labChecked("cbc","wbc")) { var w=labNumber("cbc","wbc"); if (w>11000) set.add("lab:WBC>11000"); if (w<4000) set.add("lab:WBC<4000"); }
    if (labChecked("cbc","eos")) { var e=labNumber("cbc","eos"); if (e>=10) set.add("lab:Eo>=10"); if (e>5) set.add("lab:Eo>5"); }
    if (labChecked("cbc","neut")) { var n=labNumber("cbc","neut"); if (n>75) set.add("lab:Neut>75"); }
    if (labChecked("cbc","hb")) { var hb=labNumber("cbc","hb"); if (hb<10) set.add("lab:Hb<10"); if ((p3.cbc.hb.detail||"").includes("↓≥2-3")) set.add("lab:Hb↓≥2-3g/dL/48h"); }
    if (labChecked("cbc","plt")) { var p=labNumber("cbc","plt"); if (p<100000) set.add("lab:Plt<100k"); if (p<150000) set.add("lab:Plt<150k"); }
    if (labChecked("cbc","atypical")) { var a=labNumber("cbc","atypical"); if (a>0) set.add("lab:AtypicalLym"); }
    ...
    if (labChecked("rft","cre")) { if ((p3.rft.cre.detail||"").match(/(rise|increase|≥0\.3|1\.5x)/i)) set.add("lab:CrRise"); }
    if (labChecked("rft","egfr")) { var eg=labNumber("rft","egfr"); if (!isNaN(eg)&&eg<60) set.add("lab:eGFR<60"); }
    if (labChecked("ua","protein")) set.add("lab:UA:protein+");
    if (labChecked("lung","spo2")) { var sp=labNumber("lung","spo2"); if (sp<94) set.add("sys:SpO2<94"); }
    if (labChecked("heart","ekg")) set.add("lab:EKGผิดปกติ");
    if (labChecked("heart","tropi")) { var ti=labNumber("heart","tropi"); if (ti>0.04) set.add("lab:TropI>0.04"); }
    if (labChecked("heart","tropt")) { var tt=labNumber("heart","tropt"); if (tt>0.01) set.add("lab:TropT>0.01-0.03"); }
    ...
    if (p3.immuno?.ige?.checked) {
      var igd=(p3.immuno.ige.detail||"").toLowerCase();
      if (igd.includes("igg+")) set.add("lab:IgG+");
      if (igd.includes("ldh")) set.add("lab:LDHสูง");
    }

    return set;
  }

  // ===== คำนวณคะแนนตาม tokens =====
  ...
  // ===== ซ่อนบล็อก “กราฟคะแนนย่อย (Top signals)” ถ้ามี =====
  function hideMiniSignals() {
    // ลองหา container ที่ขึ้นหัวข้อดังกล่าวแล้วซ่อน (ไม่กระทบส่วนอื่น)
    var candidates = Array.from(document.querySelectorAll("*"))
      .filter(el => /กราฟคะแนนย่อย/i.test(el.textContent || ""));
    candidates.forEach(el => {
      var card = el.closest(".p6-card") || el.closest("section") || el;
      if (card && card.parentElement) card.parentElement.removeChild(card);
    });
  }

  // ===== เรนเดอร์ผล (แสดง % และ “ทุก ADR”) =====
  function renderResults(results) {
    // ใช้ทุก ADR ทั้งหมด ไม่ slice
    var list = results;
    var max = list[0] ? Math.max(1, list[0].score) : 1;
    ...
    var html = `
      <div class="p6-card" style="background:linear-gradient(180deg,#ffeaf4 0%,#fff7fb 60%,#ffffff 100%);border:1px solid rgba(236,72,153,.25);border-radius:1.2rem;padding:1rem 1rem 1.2rem;box-shadow:0 10px 24px rgba(236,72,153,.15);">
        <h3 style="margin:0 0 .6rem;font-weight:800;color:#9d174d;">📊 สรุปคะแนนความสอดคล้อง (21 ADR — คิดเป็น %)</h3>
    ...
    hideMiniSignals(); // ซ่อนบล็อก "กราฟคะแนนย่อย" ถ้ามี
  }
