...
(function () {
  ...
  // ===== 1) ใส่สไตล์เฉพาะหน้า 6 แบบไม่ต้องแก้ style.css =====
  const STYLE_ID = "p6-page-style";
  if (!document.getElementById(STYLE_ID)) {
    ...
    st.textContent = `
      ...
      /* สีกรอบแต่ละส่วน */
      .p6-block.sec1 { border-color: rgba(59,130,246,.35); background: linear-gradient(180deg,#e0edff 0%, #ffffff 45%); }
      .p6-block.sec2 { border-color: rgba(249,115,22,.35); background: linear-gradient(180deg,#fff1e6 0%, #ffffff 50%); }
      .p6-block.sec3 { border-color: rgba(139,92,246,.25); background: linear-gradient(180deg,#f5f3ff 0%, #ffffff 52%); }
      .p6-block.sec4 { border-color: rgba(16,185,129,.25); background: linear-gradient(180deg,#dcfce7 0%, #ffffff 55%); }
      .p6-subcard {
        background:#fff;
      ...
      .p6-muted { font-size:.82rem; color:#64748b; }
      .p6-pill {
        display:inline-flex;
        align-items:center;
        gap:.35rem;
      ...
      .p6-naranjo-name { font-weight:600; color:#064e3b; }
      .p6-naranjo-score {
        font-weight:700;
        background:#fff;
      ...
      .p6-btn-print {
        background:linear-gradient(135deg,#f9c5d1 0%, #f78fb3 100%);
        color:#fff;
      }
      .p6-btn-next {
      ...
      @media (max-width: 820px) {
        .p6-footer-btns { flex-direction:column; }
      }
    `;
    document.head.appendChild(st);
  }

  // ===== 2) ตัวช่วยอ่านข้อมูลจากหน้า 1–5 แบบทน ๆ =====
