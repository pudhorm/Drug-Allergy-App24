...
<div class="p1-wrapper">
  ...
  <section class="p1-section">
    ...
    <!-- 1.11 ตำแหน่งที่พบ / การกระจายตัว (+ มิวโคซา >1) -->
    <div class="p1-block">
      ...
      <!-- ตัวเลือกใหม่: จำนวนผื่นบริเวณเยื่อบุ > 1 -->
      <div class="p1-col" style="margin-top:.6rem;">
        <label class="p1-chk">
          <input type="checkbox" id="mucosal_gt1" ${d.mucosalCountGt1 ? "checked" : ""}>
          <span>จำนวนผื่นบริเวณเยื่อบุ &gt; 1</span>
        </label>
      </div>

      <label style="margin-top:.6rem;">การกระจายตัว
...
`;
    ...
    document.getElementById("p1_save").addEventListener("click", () => {
      ...
      // ----- 1.11 ตำแหน่ง / การกระจายตัว + มิวโคซา >1 -----
      store.locations = LOCS.filter(loc => document.getElementById("loc_" + loc).checked);
      store.mucosalCountGt1 = !!document.getElementById("mucosal_gt1").checked;
